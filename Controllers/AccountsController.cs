...
namespace Controllers
{
    public class AccountsController : Controller
    {
        ...
        public ActionResult Logout(string message = "")
        {
            ...
            {
                // 1. On récupère l'utilisateur actuel dans la DB
                User userToDisconnect = DB.Users.Get(Models.User.ConnectedUser.Id);

                if (userToDisconnect != null)
                {
                    // 2. On le met hors ligne officiellement
                    userToDisconnect.Online = false;
                    DB.Users.Update(userToDisconnect);
                }

                // 3. On vide la variable de session
        ...
        {
            // On ne touche à la session QUE SI on reçoit explicitement quelque chose via l'URL
            // (Par exemple lors d'un Redirect d'une autre page)
            if (!string.IsNullOrEmpty(message))
            {
                Session["LoginMessage"] = message;
                Session["LoginSuccess"] = success;
            }
            // Si l'URL est vide, on laisse ce qu'il y a déjà en Session (comme le message du Logout)

            // Préparation du modèle
            if (Session["CurrentLoginEmail"] == null) Session["CurrentLoginEmail"] = "";
            LoginCredential credential = new LoginCredential { Email = (string)Session["CurrentLoginEmail"] };
